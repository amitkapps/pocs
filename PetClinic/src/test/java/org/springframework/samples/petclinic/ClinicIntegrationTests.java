 ...
 * @author Jim Moore
 */
@ContextConfiguration(locations={"classpath:/META-INF/spring/petclinic-context.xml"})
public class ClinicIntegrationTests extends AbstractTransactionalJUnit4SpringContextTests {
	...
	public void getPetTypes() {
		...
		assertEquals("JDBC query must show the same number of pet types", super.countRowsInTable("TYPES"), petTypes
				.size());
	...
	public void loadOwner() {
		// Load the owner in a transaction, and roll it back,
		// disconnecting the owner from the data store.
		TransactionTemplate transactionTemplate = new TransactionTemplate(transactionManager);
		Owner owner = (Owner) transactionTemplate.execute(new TransactionCallback() {
