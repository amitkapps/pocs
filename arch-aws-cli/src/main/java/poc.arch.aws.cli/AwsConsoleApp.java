...
public class AwsConsoleApp {
    ...
    private static void init() throws Exception {
        ...
        System.out.println("Environment:"+PROFILE_NAME);
...
//        sdb = new AmazonSimpleDBClient(credentials);
    ...
    private static void describeEC2Instances() {
    /*
     * Amazon EC2
     *
     ...
     */
        try {
            ...
            for ( Instance instance : instances){
                //get all devices except root
                for ( InstanceBlockDeviceMapping deviceMapping : instance.getBlockDeviceMappings()){
                    String rootDeviceName = instance.getRootDeviceName();
                    if(!deviceMapping.getDeviceName().equals(rootDeviceName)){
                        ...
                        for(Tag tag : tags){
                            if("name".equalsIgnoreCase(tag.getKey())){
                                name = tag.getValue();
                            }else if("Purpose".equalsIgnoreCase(tag.getKey()))
                                purpose= tag.getValue();
                        }
                        //is device encrypted?
                        String volumeId = deviceMapping.getEbs().getVolumeId();
                        DescribeVolumesResult describeVolumesResult = ec2.describeVolumes(new DescribeVolumesRequest().withVolumeIds(volumeId));
                        Boolean isEncrypted = describeVolumesResult.getVolumes().get(0).isEncrypted();

                        System.out.println("Env:"+PROFILE_NAME + ",Ec2 InstanceId: " + instance.getInstanceId() + ",Ec2 Name:" + name + ",Purpose:" + purpose + ", Non Root Volume Name:" + deviceMapping.getDeviceName() + ",VolumeId: " + volumeId + ", Encrypted?:"+isEncrypted);
                    }
                }
            }
    ...
    private static void getS3Details() {
    /*
     * Amazon S3
     *
    ...
    private static void getSimpleDBDetails() {
    /*
     * Amazon SimpleDB
     *
